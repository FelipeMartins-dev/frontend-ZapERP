...
const ChatListRows = memo(function ChatListRows({
  ...
  const virtualizer = useVirtualizer({
    ...
    /* Altura inicial próxima do min-height real (76/82px) — overestimate deixava vãos até o measure. */
    estimateSize: () => (isMobileLayout ? 88 : 80),
    gap: 2,
  ...
  useLayoutEffect(() => {
    ...
    const chain = () => {
      remeasure();
      frames += 1;
      if (frames < 6) rafId = requestAnimationFrame(chain);
    };
    rafId = requestAnimationFrame(chain);
    const t1 = window.setTimeout(remeasure, 80);
    const t2 = window.setTimeout(remeasure, 240);
    return () => {
